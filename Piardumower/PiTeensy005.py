...
def ButtonFlashDue_click():
    ...
    send_pfo_message('h05','1','2','3','4','5','6',)
...
def decode_message(message):  #decode the nmea message
    #KeyboardFocusManager.getCurrentKeyboardFocusManager().getActiveWindow()
            ...
            if message.sentence_type =='GPS': #it's the GPS teensy message
                txtGpsRecu.insert('1.0', str(message)+ '\n')
                locx=int(10*float(message.gpsX))
                locy=int(10*float(message.gpsY))
                

                ligne=map1.create_line(mymower.oldGpsX, mymower.oldGpsY,  locx, locy)
                mymower.oldGpsX= locx
                
                mymower.oldGpsY= locy
                map1.update()
                
                
                if firstFixFlag is False: # If we haven't found a fix before, then set the filename prefix with GPS date & time.
                    firstFixDate = message.millis 
                    firstFixFlag = True
                
                else: # write the data to a simple log file and then the raw data as well:
                    with open(cwd+"/gpsdata/" + firstFixDate +".txt", "a") as myfile:
                        myfile.write(message.gpsX + "," + message.gpsY +"\n")
    ...
    if DueConnectedOnPi :
        if myOS == "Linux":
            if os.path.exists('/dev/ttyACM0') == True:
                Due_Serial = serial.Serial('/dev/ttyACM0',115200,timeout=1,write_timeout = 1)
...
ButtonFlashDue.configure(text="Update Teensy Firmware")
...
#ligne=map1.create_line(15, 25, 200, 25)
map1.place(x=5, y=35)
