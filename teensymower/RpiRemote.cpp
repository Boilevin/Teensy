...
void RpiRemote::run() {
  readPi();
  if ((millis() >= nextTimeRaspberryPISendStat) && (millis() >= 35000)) { // start to send the stat only after 35 sec on power up to wait pi start and receive all setting
...
void RpiRemote::receivePiReqSetting (String Setting_page, int nb_page) {
  ...
  if (Setting_page == "All") {
    ...
    lineToSend = lineToSend + ",";
    lineToSend = lineToSend + robot->checkDockingSpeed;
    lineToSend = lineToSend + ",";
    lineToSend = lineToSend + "0";
    lineToSend = lineToSend + ",";
    writePi(lineToSend);
...
void RpiRemote::RaspberryPISendStat () {
  ...
  lineToSend = lineToSend + ",";
  lineToSend = lineToSend + robot->temperatureTeensy;
  lineToSend = lineToSend + ",";
